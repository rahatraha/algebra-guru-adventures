  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {subjects.map(subject => <Link key={subject.id} to={`/${subject.id}`} className="group bg-white rounded-lg shadow-sm p-6 transition-all duration-300 hover:shadow-md">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-2xl font-bold text-gray-900">
                  <span className="mr-2">{subject.icon}</span>
                  {subject.name}
                </h3>
                <Star className="h-6 w-6 text-edu-accent bg-slate-200 hover:bg-slate-100" />
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-4">
            ...
            <Link to="/daily-challenge" className="block w-full bg-edu-primary text-white rounded-lg py-3 px-6 font-medium hover:bg-edu-secondary transition-colors duration-200 text-center">
              Начать вызов
            </Link>
          </div>
        </div>
